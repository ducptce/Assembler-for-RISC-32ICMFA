# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 14:17:53 2024
...
def FileSetup(a):
    ...
    # Sử dụng regex để thay thế các ký tự không mong muốn
    a = [re.sub(r'[#/(),]', '', line).split() for line in a]

    # Loại bỏ các dòng trống và các chỉ thị không cần thiết
    ...
    while i < len(a):
        if a[i][0].endswith(":"):
            label[a[i][0][:-1]] = idx
            a[i].append(idx)
            # Kiểm tra kiểu dữ liệu trước khi gọi upper()
            if isinstance(a[i][1], str) and a[i][1].upper().startswith("C."):
                idx += 2
            else:
                idx += 4
            a[i].pop(0)
            ...
            a[i].append(idx)
            if isinstance(a[i][0], str) and a[i][0].upper().startswith("C."):
                idx += 2
            else:
                idx += 4
        i += 1

    # Chuyển tất cả mã lệnh thành chữ in hoa
    for i in range(len(a)):
        if isinstance(a[i][0], str):  # Check if it's a string before calling upper()
            a[i][0] = a[i][0].upper()
        else:
            print(f"Warning: Expected a string but got {type(a[i][0])} at index {i}")
...
def scan_file(a, out, a32):
    flag = 0
    if os.path.isdir(a):
        os.chdir(a)  # Chuyển vào thư mục
        print(f"Processing directory: {a}")

        List = os.listdir()
        if len(List) != 0:
            for i in List:
                if i.endswith('.S'):  # Chỉ xử lý tệp .S
                    out_dir = os.path.join(out, os.path.basename(a))  # Tạo thư mục đầu ra
                    if not os.path.exists(out_dir):
                        os.mkdir(out_dir)
                    # Gọi hàm xử lý tệp .S
                    scan_file(os.path.join(a, i), out_dir, a32)
        os.chdir(os.path.dirname(a))  # Quay lại thư mục cha
    elif a.endswith('.S'):
            print(f"Processing file: {a}")
            ins = []
            d = a.replace(os.getcwd().replace('\\', '/') + '/', '')
            with open(a, 'r') as f:
                a = f.readlines()
                ...
                while i < len(temp):
                    if len(temp[i]) == 16:
                        if i == len(temp) - 1:
                            ...
                            break
                        if len(temp[i + 1]) == 32:
                            ins += temp[i + 1][16:32] + temp[i] + '\n'
                            i += 1
                        elif len(temp[i + 1]) == 16:
                            ins += temp[i + 1][:16] + temp[i] + '\n'
                            i += 2
                    else:
                        if flag:
                            if i == len(temp) - 1:
            ...
            with open(os.path.join(out, f'{d}.bin'), 'w') as f:
                f.write('\n'.join(ins) + '\n')
    ...
    if flag == 0:
        ...
        if len(List) != 0:
            ...
            for i in List:
                if i.find('.S') != -1:
                    out += f'/{os.path.basename(os.getcwd())}'
...
if __name__ == "__main__":
    ...
    # change to directory of output
    os.chdir(_out)
    
    scan_file(_in, _out, a32)